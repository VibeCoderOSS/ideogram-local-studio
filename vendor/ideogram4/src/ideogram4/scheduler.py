...
class LogitNormalSchedule:
  ...
  def __call__(self, t: torch.Tensor) -> torch.Tensor:
    if t.device.type == "mps":
      # MPS has no float64 support. The pipeline immediately reads these
      # schedule values as Python scalars, so CPU evaluation preserves the
      # original float64 math without putting unsupported dtype work on Metal.
      t = t.cpu()
