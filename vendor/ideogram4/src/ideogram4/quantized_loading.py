...
def load_fp8_state_dict(
  ...
  *,
  ...
  """
  dequantize_fp8_for_mps = device.type == "mps"
  prepared: dict[str, torch.Tensor] = {}
  for k, v in state_dict.items():
    if v.dtype == FP8_WEIGHT_DTYPE:
      if dequantize_fp8_for_mps:
        if not k.endswith(".weight"):
          raise RuntimeError(f"unexpected fp8 tensor key for MPS dequantization: {k}")
        scale_key = k.removesuffix(".weight") + FP8_SCALE_SUFFIX
        scale = state_dict.get(scale_key)
        if scale is None:
          raise RuntimeError(f"missing fp8 scale for {k}: expected {scale_key}")
        prepared[k] = (v.to(dtype) * scale.to(dtype).unsqueeze(1)).to(device=device)
      else:
        prepared[k] = v.to(device=device)
    elif k.endswith(FP8_SCALE_SUFFIX):
      if dequantize_fp8_for_mps:
        prepared[k] = torch.ones(v.shape, device=device, dtype=torch.float32)
      else:
  ...
  missing, unexpected = model.load_state_dict(
    prepared, strict=False, assign=assign or dequantize_fp8_for_mps
  )
  if unexpected:
